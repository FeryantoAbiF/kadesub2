...
class FavoritePrevAdapter(
    private val match: List<FavoritePrev>,
    private val context: Context,
    private val listener: (FavoritePrev) -> Unit
) : RecyclerView.Adapter<FavoriteHolder>() {
    override fun onCreateViewHolder(viewGroup: ViewGroup, p1: Int): FavoriteHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.item_event, viewGroup, false)
...
class FavoriteHolder(itemview: View) : RecyclerView.ViewHolder(itemview) {
    ...
    fun bindItem(teams: FavoritePrev, listener: (FavoritePrev) -> Unit) {
